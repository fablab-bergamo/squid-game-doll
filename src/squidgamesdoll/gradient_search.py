...
def group_candidates(candidates, group_radius=5):
    ...
    while candidates:
        ...
        grouped.append({'position': (int(round(avg_x)), int(round(avg_y))),
                        'weight': total_weight})
...
def detect_laser_spots(image, R=10, Th=20):
    ...
    candidates = [{'position': (int(x), int(y)), 'weight': int(w)}
...
def draw_candidates(image, candidates):
    ...
    for candidate in candidates:
        ...
        cv2.putText(output, str(weight), (x+5, y-5),
...
def test_gradient(image: cv2.UMat) -> list:
    ...
    for idx, candidate in enumerate(candidates):
        print("Candidate {}: position={}, weight={}".format(
    ...
    for idx, candidate in enumerate(grouped_candidates, start=1):
        print("Grouped Candidate {}: position={}, weight={}".format(
