...
def find_laser(img: cv2.UMat, strategy_hint: str, threshold_hint: int) -> (tuple, cv2.UMat, str, int):
    ...
    """
    strategies = [find_laser_by_red_color, find_laser_by_grayscale, find_laser_by_red_color_adapt]
    ...
    for strategy in strategies:
        ...
        if strategy_hint is not None and strategy.__name__ != strategy_hint:
            hint = None
        else:
            hint = threshold_hint
       
        (coord, output, threshold) = strategy(img.copy(), hint)
...
def find_laser_by_hsv(img: cv2.UMat, threshold_hint:int) -> (tuple, cv2.UMat, int):
    # Define HSV range for detecting red laser (adjust if needed)
    ...
    while tries < MAX_TRIES:
        lower_red = np.array([0, threshold, 78])
        upper_red = np.array([10, 255, 255])
        mask = cv2.inRange(hsv, lower_red, upper_red)
     
        masked_channel = cv2.dilate(mask, None, iterations=4)
...
def find_laser_by_threshold(channel: cv2.UMat, threshold_hint:int) -> (tuple, cv2.UMat, int):
    ...
    while tries < MAX_TRIES:
        ...
        circles = cv2.HoughCircles(masked_channel, cv2.HOUGH_GRADIENT, 1, minDist=50,
                                param1=50,param2=2,minRadius=3,maxRadius=15)
        ...
        if circles_cpt == 0:
            ...
            print(f"Found no circles, decreasing threshold to {threshold}")
            if threshold < MIN_THRESHOLD:
                return (None, masked_channel, None)
            tries += 1
            continue
...
def find_laser_by_adaptative_threshold(channel: cv2.UMat, threshold_hint:int) -> (tuple, cv2.UMat, int):
    """
    Finds the laser in the given channel using a thresholding strategy.
    ...
    tries = 0
    while tries < MAX_TRIES:
        diff_thr = cv2.adaptiveThreshold(channel, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 7, 2)
        diff_thr = cv2.bitwise_not(diff_thr)
        cv2.imshow("Threshold", cv2.cvtColor(diff_thr, cv2.COLOR_GRAY2BGR))
        ...
        circles = cv2.HoughCircles(masked_channel, cv2.HOUGH_GRADIENT, 1, minDist=50,
                                param1=50,param2=2,minRadius=3,maxRadius=20)
...
def find_laser_by_contours(channel: cv2.UMat, threshold_hint:int) -> (tuple, cv2.UMat, int):
    """
    Finds the laser in the given channel using a thresholding strategy.
    ...
    while tries < MAX_TRIES:
        ...
        contours, hierarchy = cv2.findContours(masked_channel, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

        
        circles = cv2.HoughCircles(masked_channel, cv2.HOUGH_GRADIENT, 1, minDist=50,
                                param1=50,param2=2,minRadius=4,maxRadius=20)
...
def find_laser_by_red_color_adapt(img: cv2.UMat, hint:int) -> (tuple, cv2.UMat, int):
    """
    Finds the laser in the given image using the red color channel.
    ...
    """
    (_, _, R) = cv2.split(img)
    return find_laser_by_adaptative_threshold(R, hint)
